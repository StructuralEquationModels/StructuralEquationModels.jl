...
function eachrow_to_col(::Type{T}, column_indices::AbstractVector{Int}, ncolumns::Integer) where T
    nrows = length(column_indices)
    (nrows > ncolumns) &&
        throw(DimensionMismatch("The number of rows ($nrows) cannot exceed the number of columns ($ncolumns)"))
    all(i -> 1 <= i <= ncolumns, column_indices) ||
        throw(ArgumentError("All column indices must be between 1 and $ncolumns"))

    sparse(eachindex(column_indices), column_indices,
           ones(T, nrows), nrows, ncolumns)
end
...
function eachcol_to_row(::Type{T}, row_indices::AbstractVector{Int}, nrows::Integer) where T
    ncols = length(row_indices)
    (ncols > nrows) &&
        throw(DimensionMismatch("The number of columns ($ncols) cannot exceed the number of rows ($nrows)"))
