...
#=     in_S = length.(S_ind) .!= 0
    ...
    for (i, S_ind) in enumerate(S_ind)
        for c_ind in C_indices[S_ind]
            ...
            start_val[i] = (pos <= length(F_ind)) && (F_ind[pos] == c_ind[1]) ?
                Σ[pos, pos]/2 : 0.05
    ...
    for i ∈ setdiff(1:n_var, F_ind)
        ...
        if length(reference) > 0
            ...
            for (j, indicator) in enumerate(indicators)
                if (indicator != ref) && (parampos = get(indicator2parampos, indicator, 0)) != 0
        ...
        elseif length(indicators) > 0
            ref = indicators[1]
            λ = Vector{Float64}(undef, length(indicators)); λ[1] = 1.0
            for (j, indicator) in enumerate(indicators)
                if indicator != ref
