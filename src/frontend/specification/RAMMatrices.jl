...
function observed_vars(ram::RAMMatrices)
    if isnothing(ram.colnames)
        ...
        return [col for (i, col) in enumerate(ram.colnames) if isobserved_var(ram, i)]
    end
end
...
function RAMMatrices(
    ...
    if length(partable.variables.sorted) != 0
        ...
        F_inds = findall(∈(Set(partable.variables.observed)), colnames)
    else
        colnames = [
    ...
    for row in partable
        ...
        if !row.free
            ...
            elseif (row.relation == :→)
                push!(
                    A_consts,
                    A_lin_ixs[CartesianIndex(row_ind, col_ind)] => row.value_fixed,
                )
            elseif (row.relation == :↔)
                push!(
                    S_consts,
                    S_lin_ixs[CartesianIndex(row_ind, col_ind)] => row.value_fixed,
                )
                if row_ind != col_ind # symmetric
                    push!(
                        S_consts,
                        S_lin_ixs[CartesianIndex(col_ind, row_ind)] => row.value_fixed,
                    )
                end
            else
...
function ParameterTable(
    ...
    append_rows!(partable, ram.S, :S, ram.params, position_names, skip_symmetric = true)
    append_rows!(partable, ram.A, :A, ram.params, position_names)
    if !isnothing(ram.M)
        append_rows!(partable, ram.M, :M, ram.params, position_names)
    end
...
function append_rows!(
    ...
    nparams(arr) == length(params) || throw(
        ArgumentError(
            "Length of parameters vector does not match the number of parameters in the matrix",
        ),
    )
    ...
    for (i, par) in enumerate(params)
        for j in param_occurences_range(arr, i)
            ...
            push!(
                partable,
                partable_row(par, arr_ix, arr_name, position_names, free = true),
            )
            if skip_symmetric
