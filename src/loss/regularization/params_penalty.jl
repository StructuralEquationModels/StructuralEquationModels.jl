...
transform `A * p + b`, with fixed `A` and `b`.

Without a hinge bound (`bound = :none`), the elementwise penalty is
``|z_i|^{s_1} + w_2 * |z_i|^{s_2}``. The second term is optional.

With `bound = :l` or `bound = :u`, the above penalty is applied to the
one-sided hinge-transformed *z* instead (``z = \\max(x - t, 0)``, or
``z = \\max(t - x, 0)``, respectively, where *t* is the `threshold` parameter).

In addition to the generic `SemParamsPenalty` constructor, `SemNorm`,
`SemLasso`, `SemRidge`, `SemElasticNet` and `SemHinge` wrappers provide
convenient construction of common regularization penalties.

# Constructors
    SemParamsPenalty([params], [A], [b];
                     shape1 = 1, shape2 = nothing, weight2 = nothing,
                     bound = :none, threshold = 0.0)
...
function SemParamsPenalty(
    ...
    isnothing(param_inds) ||
        !(eltype(param_inds) <: Symbol) ||
    ...
    if !isnothing(b)
        (!isnothing(A) || !isnothing(param_inds)) || throw(
            ArgumentError(
    ...
    shape1 = shape1 isa AbstractFloat && isinteger(shape1) ? Int(shape1) : shape1
    shape2 =
        isnothing(shape2) ? nothing :
        (shape2 isa AbstractFloat && isinteger(shape2) ? Int(shape2) : shape2)
...
function SemParamsPenalty(
    ...
    return SemParamsPenalty(param_inds, A, b; shape1, shape2, weight2, threshold, bound)
end

# wrappers for common regularization schemes
...
SemElasticNet(args...; weight2::Real = 0.5) =
    SemParamsPenalty(args...; shape1 = 1, shape2 = 2, weight2)
SemHinge(args...; bound::Symbol = :l, kwargs...) =
    SemParamsPenalty(args...; bound, kwargs...)

nparams(f::SemParamsPenalty) =
    !isnothing(f.A) ? size(f.A, 2) : (isnothing(f.param_inds) ? 0 : length(f.param_inds))

# helpers using constant propagation for efficient computation
# for specific shapes and bounds
...
elnormhdiag(x, ::Val{2}, mode, threshold) =
    _elhinge_isactive(x, mode, threshold) ? (one(x) + one(x)) : zero(x)

elnormhdiag(x, ::Val{S}, mode, threshold) where {S} =
    _elhinge_isactive(x, mode, threshold) ?
    S * (S - 1) * _elhinge(x, mode, threshold)^(S - 2) : zero(x)

Base.@propagate_inbounds @inline function _elevaluate!(
    objective,
    elm_grad,
    elm_hdiag,
    x,
    ...
    isnothing(elm_hdiag) ||
        (elm_hdiag[i] += weight * elnormhdiag(x, shape, mode, threshold))
...
function _evaluate!(
    ...
    gradient,
    hessian,
    loss::SemParamsPenalty,
    params,
    shape1::Val{S1},
    shape2::Val{S2},
    mode::Val{H},
) where {S1, S2, H}
    sel_params = isnothing(loss.param_inds) ? params : params[loss.param_inds]
    trf_params = isnothing(loss.A) ? sel_params : loss.A * sel_params
    if !isnothing(loss.b)
        trf_params === params && (trf_params = copy(trf_params))
        trf_params .+= loss.b
    end

    weight2 = isnothing(loss.weight2) ? zero(eltype(trf_params)) : loss.weight2
    ...
    if !isnothing(gradient)
        if isnothing(loss.A)
            if isnothing(loss.param_inds)
                copyto!(gradient, elm_grad)
            else
                fill!(gradient, zero(eltype(gradient)))
                @inbounds gradient[loss.param_inds] .= elm_grad
            end
        elseif isnothing(loss.param_inds)
            mul!(gradient, loss.At, elm_grad)
        else
            local_grad = loss.A' * elm_grad
            fill!(gradient, 0)
            @inbounds gradient[loss.param_inds] .= local_grad
        end
    end
    ...
    if !isnothing(hessian)
        ...
        if !all(iszero, elm_hdiag)
            @inbounds if isnothing(loss.A)
                if isnothing(loss.param_inds)
                    hessian[diagind(hessian)] .= elm_hdiag
                else
                    hessian[diagind(hessian)[loss.param_inds]] .= elm_hdiag
                end
            else
                local_hessian = loss.A' * Diagonal(elm_hdiag) * loss.A
                if isnothing(loss.param_inds)
                    copyto!(hessian, local_hessian)
                else
                    hessian[loss.param_inds, loss.param_inds] .= local_hessian
                end
            end
        end
    end
...
evaluate!(objective, gradient, hessian, loss::SemParamsPenalty{H}, params) where {H} =
    _evaluate!(
        objective,
        gradient,
        hessian,
