...
Base.@kwdef struct ForwardBackwardIteration{R,Tx,Tf,Tg}
    f::Tf
    g::Tg
    x0::Tx
    adaptive::Bool = true
...
function Base.iterate(iter::ForwardBackwardIteration)
    x = copy(iter.x0)
    f_x, grad_f_x = objective_gradient!(iter.f, x)
    gamma = lower_bound_smoothness_constant(iter.f, I, x, grad_f_x)
    y = x - gamma .* grad_f_x
...
function Base.iterate(iter::ForwardBackwardIteration{R}, state::ForwardBackwardState{R,Tx}) where {R,Tx}
    if iter.adaptive == true
        ...
        state.x, state.z = state.z, state.x
        state.f_x = _gradient!(state.grad_f_x, iter.f, state.x)
    end
...
struct ForwardBackward{R, K}
    ...
    verbose::Bool 
    freq::Int # frequency of messages if verbose = true

    kwargs::K # passed to ProximalGradientIteration
end
