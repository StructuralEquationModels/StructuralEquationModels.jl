...
#PoximalOperators.is_quadratic(model::Sem) = false 
...
function Base.iterate(iter::PANOCIteration{R}) where R
    x = copy(iter.x0)
    Ax = iter.A * x
    f_Ax, grad_f_Ax = objective_gradient!(iter.f, Ax)
...
function Base.iterate(iter::PANOCIteration{R}, state::PANOCState) where R
    ...
    state.f_Ax_d = objective_gradient!(state.grad_f_Ax_d, iter.f, state.Ax_d)
...
#=             if isinf(f_Az)
                f_Az = objective_gradient!(state.grad_f_Az, iter.f, state.Az)
            end
            if isinf(c)
            ...
            state.f_Ax = objective_gradient!(state.grad_f_Ax, iter.f, state.Ax)
