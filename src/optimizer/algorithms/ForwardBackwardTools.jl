...
function lower_bound_smoothness_constant(f, A, x, grad_f_Ax)
    R = real(eltype(x))
    xeps = x .+ 1
    grad_f_Axeps = copy(gradient!(f, A * xeps))
    print(norm(A' * (grad_f_Axeps - grad_f_Ax)) / R(sqrt(length(x))))
...
function _gradient!(y, f, x)
    obj, grad = gradient!(f, x)
    y .= grad
    return obj
end

_gradient!(::Nothing, f, x) = objective!(f, x)
...
function backtrack_stepsize!(
    ...
    Ax = A * x
    f_Ax, grad_f_Ax = objective_gradient!(f, Ax)
