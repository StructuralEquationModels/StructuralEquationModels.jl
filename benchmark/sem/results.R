...
results %>%
  select(starts_with("mean_time"), missingness, n_factors, n_items) %>% 
  pivot_longer(starts_with("mean_time"), names_to = "package", values_to = "mean_time") %>% 
  ggplot(aes(x = n_factors*n_items, y = mean_time, color = as.factor(missingness))) +
  geom_point() + theme_minimal() + geom_line(aes(linetype = package)) +
  scale_x_log10() + scale_y_log10()

results %>%
  select(starts_with("mean_time"), missingness, n_factors, n_items) %>% 
  filter(missingness == 0.2) %>% 
  pivot_longer(starts_with("mean_time"), names_to = "package", values_to = "mean_time") %>% 
  ggplot(aes(x = n_factors*n_items, y = mean_time)) +
  geom_point() + theme_minimal() + geom_line(aes(linetype = package))
